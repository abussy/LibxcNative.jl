...
function evaluate_gga!(func::Functional, params::NamedTuple, n_spin::Int,
    ...
    if n_spin == 1
        return evaluate_gga_unp!(func, params, rho, sigma, out_zk, out_vrho, out_vsigma)
    else
        return evaluate_gga_pol!(func, params, rho, sigma, out_zk, out_vrho, out_vsigma)
    end
end

function evaluate_gga_unp!(func::Functional, params::NamedTuple,
                           rho::AbstractMatrix, sigma::AbstractMatrix,
                           out_zk, out_vrho, out_vsigma)
    n_points = size(rho, 2)
    T = eltype(rho)
...
function evaluate_gga_pol!(func::Functional, params::NamedTuple,
                           rho::AbstractMatrix, sigma::AbstractMatrix,
                           out_zk, out_vrho, out_vsigma)
    n_points = size(rho, 2)
    T = eltype(rho)
